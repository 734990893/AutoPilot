...
class Net(nn.Module):
    ...
    # 3 dim output
    def action_decode(self, z, speed):
        
        #print(z.shape, speed.shape)
        z = torch.cat((z, speed), 1)
        #print(z.shape)
        z = F.relu(self.fc1(z))
        z = F.relu(self.fc2(z))
    ...
    def forward(self, x, speed, mode='default'):
        #if self.training:
        z = self.encode(x)
        ah = self.action_decode(z, speed)
