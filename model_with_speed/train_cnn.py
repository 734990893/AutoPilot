...
if __name__ == "__main__":
    ...
    data_rgb, data_action_exp = load_data(raw_data_path, 0, iteration+1, 1, 30) # iteration+1
    
    print(data_action_exp.shape) # 2900, 3
    ...
    for epoch in range(nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            tloss= loss_function(ah, action_exp[:, :3])
            tloss.backward()
            optimizer.step()

            total_loss_ephoc[0] += tloss.detach().tolist() #/ target.shape[0]
            #total_loss_ephoc[1] += MSEa.detach().tolist() #/ target.shape[0]
            #total_loss_ephoc[2] += MSEr.detach().tolist() #/ target.shape[0]
            #total_loss_ephoc[3] += kl_loss.detach().tolist()
